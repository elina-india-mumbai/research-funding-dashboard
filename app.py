...
# ---------- Session state for click-to-filter ----------
...
if not df_state.empty and "aggregated_amount" in df_state.columns:
    ...
    # Capture click via on_select (Streamlit ≥ 1.35)
    event = st.plotly_chart(
        fig_map, use_container_width=True, on_select="rerun", key="map_click"
    )
    ...
    if event and event.selection and event.selection.points:
        ...
        clicked_state = pt.get("location")

    if clicked_state:
        if st.session_state.selected_state == clicked_state:
            st.session_state.selected_state = None  # toggle off
        else:
    ...
    if selected_state:
        ...
        st.info(
            f"Showing data for **{display}**  ·  Click the state again on the map to clear filter"
        )
else:
